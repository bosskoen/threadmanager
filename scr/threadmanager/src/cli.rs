...
pub fn initialise_cli() -> HashMap<&'static str, Box<dyn Fn(&[&str], &mut Manager, &Printer)>>{
    ...
    cli.insert("start", Box::new(start_app) as Box<dyn Fn(&[&str], &mut Manager, &Printer)>);
    cli.insert("stop", Box::new(stop_app) as Box<dyn Fn(&[&str], &mut Manager, &Printer)>);
    cli.insert("status", Box::new(get_status) as Box<dyn Fn(&[&str], &mut Manager, &Printer)>);
    cli.insert("list", Box::new(list_apps) as Box<dyn Fn(&[&str], &mut Manager, &Printer)>);
    cli.insert("help", Box::new(help) as Box<dyn Fn(&[&str], &mut Manager, &Printer)>);
    cli.insert("settings", Box::new(settings) as Box<dyn Fn(&[&str], &mut Manager, &Printer)>);
    cli.insert("led", Box::new(led) as Box<dyn Fn(&[&str], &mut Manager, &Printer)>);
...
fn start_app(args: &[&str], open_threads: &mut Manager, printer: &Printer) {
    if args.len() == 0{
        printer.print("No thread name provided. Please specify a thread name.", RGB::WHITE());
        return;
    }else if args.len() > 1{
        printer.print("Multiple thread names provided. Please specify only one thread name.", RGB::WHITE());
        return;
    }
    printer.print(&format!("Attempting to start thread: {}", args[0]), RGB::WHITE());
...
fn stop_app(args: &[&str],open_threads: &mut Manager,printer: &Printer){
    ...
    if args[0] == "all"{
        ...
        printer.print("Note: 'errorThread' is a permanent system thread and cannot be stopped.", RGB::TRACE());
        #[cfg(feature = "led")]
        printer.print(&format!("{} can't be stopped this way use the manual command to stop it.", error_handeler::light_dimmer_thread::PLUGIN_NAME), RGB::TRACE());
    ...
    printer.print(&format!("Attempting to stop thread: {}", args[0]), RGB::WHITE());
...
fn list_apps(args: &[&str], open_threads: &mut Manager,printer: &Printer){
    if args.len() == 0{
        printer.print("No argument provided. Please specify 'running', 'stopped', or 'all'.", RGB::WHITE());
        return;
    }else if args.len() > 1{
        printer.print("Invalid argument. Please specify 'running', 'stopped', or 'all'.", RGB::WHITE());
        return;
    }
    match args[0] {
        ...
        _ => printer.print("Invalid argument. Please specify 'running', 'stopped', or 'all'.", RGB::WHITE())
    }
}
...
fn settings(args: &[&str], open_threads: &mut Manager,printer: &Printer) {
    if args.len() == 0 {
        printer.print("No argument provided. Available command:\n\
               reload - Reload configuration settings", RGB::WHITE());
        return;
    } else if args.len() > 1 {
        printer.print("Invalid argument. Only 'reload' is supported.", RGB::WHITE());
        return;
    }
...
#[cfg(not(feature = "led"))]
fn led(args: &[&str], open_threads: &mut Manager,printer: &Printer){
...
fn led(args: &[&str], open_threads: &mut Manager, printer: &Printer){
    if args.len() == 0 {
        ...
        printer.print( "led on [red/green/blue/all] [0-4/all]\n\
    ...
    match args[0] {
        "off" => {
            if args.len() == 3{
                ...
                match args[1] {
                    ...
                    _ => {printer.print("Invalid argument. please specify a color and LED\nled off [red/green/blue/all] [1-4/all]", RGB::WHITE());
                        return;},
                }
                match args[2]{
        ...
        "help" => {
            ...
            printer.print("led on/off/reset [red/green/blue/all] [0-4/all] - Control specific colors\n\
                   led color <hex color (0xRRGGBB)> [0-4/all] - Set LED color\n\
                   led brightness <0-16> [0-4/all] - Set brightness level",
                   RGB::WHITE());
        },
        _ => printer.print("Invalid LED command. Use 'led help' to see available commands.", RGB::WHITE()),
    }
}
